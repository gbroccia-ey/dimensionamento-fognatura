...
import { NavController, NavParams, Navbar,ModalController, Platform , LoadingController} from 'ionic-angular';
...
export class Form1Page {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, 
    ...
    if(navParams.data) {
      ...
      this.LogManager.info("form1", this.ads); 
      ...
      if(!this.ads.VerbaleDiSopralluogo.Ragione_Sociale) this.ads.VerbaleDiSopralluogo.Ragione_Sociale = this.ads.Cliente.RagioneSociale;
     ...
     // this.Avviso = this.ads._codice_avviso;
      // this.Avviso = this.ads.CodiceAds + " / " + this.ads.CodiceOdl;
      if(this.ads.CodiceAds != undefined)
        this.ads.VerbaleDiSopralluogo.Avviso = this.ads.CodiceAds
      else this.ads.VerbaleDiSopralluogo.Avviso = "";
      ...
      switch(this.ads.SettoreMerceologico)
      {
        ...
        case SettoreMerceologico.TLR:
          this.ads.VerbaleDiSopralluogo.Tlr = true;
        break;                          
      ...
      this.ads.VerbaleDiSopralluogo.Letto_in_Data = new Date().toISOString();  
  ...
  ionViewDidLoad() {
      this.LogManager.info("form1 - ionViewDidLoad"); 
  ...
  updateSelectMultiple(){
    ...
    document.getElementById('selectMultiple').children[0].innerHTML = str;
    setTimeout(function(){ 
      document.getElementById('selectMultiple').children[0].innerHTML = str;
      str = null;
    },600);
  ...
  naviga_back_details(){
     this.navCtrl.popToRoot().then(() => {
      ...
      this.navCtrl.push(HomePage,this.ads);
    });
  }
  ...
  clickFab(){
    this.zone.run(() => {
      this.showBack = !this.showBack;
          if(this.showBack==true) document.getElementById('allPageF1').className = 'glass';
    else document.getElementById('allPageF1').className = '';
    })
  }
  
  Add_Descrizione(){      
      if(this.conta_descrizioni < 3)
      {
        this.conta_descrizioni++;
        this.ads.VerbaleDiSopralluogo.show_comp.push({descrizione:"",tipo:""});
      }        
      else
       this.showAlert();
  }
  ...
  Elimina_Descrizione(){
      this.conta_descrizioni--;
      this.ads.VerbaleDiSopralluogo.show_comp.splice(this.conta_descrizioni,1);
  }
  ...
  Carica_Dati(){
        this.ads.VerbaleDiSopralluogo.Letto_in_Data = new Date().toISOString();      
          for(let key in this.ads.VerbaleDiSopralluogo) {
            if(this.ads.VerbaleDiSopralluogo[key] == " ")
              this.ads.VerbaleDiSopralluogo[key] = null;
          } 
  }
  ...
  Richiama_Salvataggio_e_pdf(){    
    ...
    else if(this.ads.Prestazione !==  Prestazione.PR1 && this.ads.DettaglioMerceologico !== DettaglioMerceologico.FOGNATURA && !this.ads.VerbaleDiSopralluogo.AdeguamentoCaricoCliente && !this.ads.VerbaleDiSopralluogo.Pratica_Annullata) {
      let alert = this.alertCtrl.create({
        title: 'Attenzione!',
    ...
    else if((this.ads.Prestazione !==  Prestazione.PR1 && this.ads.DettaglioMerceologico !== DettaglioMerceologico.FOGNATURA && this.ads.VerbaleDiSopralluogo.AdeguamentoCaricoCliente === AdeguamentoCliente.NON_ADEGUATO) &&
  ...
  Crea_pdf(){
    ...
    for(let key in this.ads.VerbaleDiSopralluogo) {
      if(!this.ads.VerbaleDiSopralluogo[key] && typeof this.ads.VerbaleDiSopralluogo[key] !== "boolean" && key!="AdeguamentoPDF"){
    ...
    console.log("FORM 1 : CREA PDF");
    

    var self = this;
    this.ads.Cliente.Telefono = this.ads.VerbaleDiSopralluogo.Telefono_Richiedente;
...
//TODO: cambia
 ...
 var pdfCreatedSuccess = (result: PdfResult) => {
    ...
    this.pdfManager.pdfCreate("ModelAcqua", item).then(
      url => {
        ...
        bean.postprocessTypology = "ModelAcqua";
        bean.pdfBean = item;
  ...
  savePhotoInGal( obj){ 
    ...
    fileUtil.createFile(this.ads.Id,'images','disegnoTecnico.jpg',blob,function(){
      ...
      for(let item of self.ads.dataFoto){
        ...
        self.ads._base64Img = containerBase64;
